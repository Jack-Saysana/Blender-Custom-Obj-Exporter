...
"""
object: Armature object being traversed
entity_origin: Entity origin in world space
bones: list to be populated with bone names
bone: current bone being processed
parent_mat: bonespace matrix of parent
parent_id: id of parent bone, -1 if no parent
obj_file: output file
"""
def traverse_armature(object, entity_origin, bones, bone, parent_mat, parent_id, obj_file):
    cur_id = len(bones)
    entity_mat = mathutils.Matrix.Translation(-entity_origin) @ object.matrix_world;
    world_head = opengl_mat @ (entity_mat @ bone.head_local)
    world_tail = opengl_mat @ (entity_mat @ bone.tail_local)
    parent_mat = parent_mat @ bone.matrix
    # Armature space to world space to opengl space
    opengl_bone_mat = opengl_mat @ (entity_mat.to_3x3() @ parent_mat)
    ...
    for child in bone.children:
        traverse_armature(object, entity_origin, bones, child, parent_mat, cur_id, obj_file)

"""
object: Mesh object to serialize
entity_mat: entity-space transformation matrix of entity armature being traversed
...
def serialize_mesh(entity_mat, object, bones, obj_file, mtl_file):
    ...
    for vertex in mesh_data.vertices:
        ...
        world_coords = opengl_mat @ (entity_mat @ vertex.co)
    ...
    for polygon in mesh_data.polygons:
        if exists(normals, polygon.normal, 3) == False:
            ...
            normal_matrix = entity_mat.to_3x3().inverted_safe().transposed()
...
obj_file: output object file
"""
def serialize_collider_collection(entity_origin, col_start, collection, bones, obj_file):
    cur_col = col_start
...
obj_file: output object file
"""
def serialize_action(action, bones, obj_file):
    keyframe_chains = {}
...
def serialize_single_entity(filepath):
    ...
    # For every armature of entity, output the bones
    bones = []
